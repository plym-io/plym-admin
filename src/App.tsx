...
/**
 * One boundary over the whole panel, sign-in included.
 *
 * Every screen is a lazily loaded chunk, so the failure worth catching is the
 * one an update causes: the files this tab is holding references to are
 * replaced, and the next navigation asks for a chunk that has stopped
 * existing. That lands on whichever route was being entered — /login as
 * readily as /posts — and without a boundary React Router says so in the
 * language of a stack trace.
 */
  ...
  [
    {
      ...
      children: [
        ...
        // Outside RequireAuth on purpose: the session it is here to establish
        // does not exist yet, and a bounce to /login would spend the code for
        // nothing.
