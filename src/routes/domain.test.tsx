...
/** Already moved, and mounted under a folder rather than at the domain root. */
const CONNECTED_UNDER_A_FOLDER: RoutingOptions = {
  placement: {
    slug: 'acme',
    platform_domain: 'plym.space',
    origin_url: 'https://acme.plym.space',
    public_host: 'www.acme.com',
    public_url: 'https://www.acme.com',
    prefix: '/blog',
    blog_home: 'https://www.acme.com/blog/',
    at_root: false,
...
describe('where the blog answers today', () => {
  it('keeps the folder it is mounted under', async () => {
    getRouting.mockResolvedValue(CONNECTED_UNDER_A_FOLDER);
    render(<Domain />);

    // `public_url` is the hostname alone, and would read as the site root.
    expect(await screen.findByText('https://www.acme.com/blog/')).toBeInTheDocument();
    expect(screen.getByText('Custom domain connected.')).toBeInTheDocument();
  });

  it('falls back to the hostname when the gateway sends no blog home', async () => {
    getRouting.mockResolvedValue(CURRENT);
    render(<Domain />);

    expect(await screen.findByText('https://acme.plym.space')).toBeInTheDocument();
  });
});
