...
export default function Domain() {
  ...
  // Where readers actually reach it, path and all. `public_url` is only the
  // hostname, so a blog mounted under a folder reads as if it sat at the root
  // — `blog_home` is the one the gateway builds with the prefix on it. The
  // origin is the proxy upstream, not an address to advertise, so it is last.
  const servedAt =
    placement?.blog_home ?? placement?.public_url ?? status?.url ?? placement?.origin_url;
