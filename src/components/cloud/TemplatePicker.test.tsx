...
describe('TemplatePicker', () => {
  ...
  it('offers an update on the template already live', async () => {
    await paint();

    const row = screen.getByText('atlas').closest('div')!;
    expect(row.querySelector('button')).toBeEnabled();
    expect(row.querySelector('button')).toHaveTextContent('Update');
    expect(screen.getByText('Live')).toBeInTheDocument();
    // The next row along is the whole point of the screen.
    expect(screen.getByText('quill').closest('div')!.querySelector('button')).toHaveTextContent(
      'Install',
    );
  });

  it('fetches the live template again rather than switching to it', async () => {
    // Its files are here, so an install would skip the fetch — but the fetch
    // is the entire point of Update: a newer copy of the template it is on.
    await paint();
    await clickInstall('atlas');
    await settle();

    expect(installTemplate).toHaveBeenCalledWith('atlas', 'public');
    expect(applySettings).not.toHaveBeenCalled();
    expect(success).toHaveBeenCalledWith('Template updated.', expect.anything());
  });

  it('says so when the update does not land', async () => {
    getOpEvents.mockResolvedValue(finished('op-install', 'failed'));
    await paint();
    await clickInstall('atlas');
    await settle();

    expect(error).toHaveBeenCalledWith('atlas could not be updated.');
  });

  it('does not claim an update landed on the evidence it was already there', async () => {
    // Every fact the catalogue could report about the live template was true
    // before the fetch ran, so none of them is proof that it ran.
    getOpEvents.mockRejectedValue({ code: 'x', message: 'gone', status: 502, raw: null });
    await paint();
    await clickInstall('atlas');
    await settle(95);

    expect(plain).toHaveBeenCalledWith('Lost sight of your blog while it restarted.');
    expect(success).not.toHaveBeenCalled();
  });

  it('links every public template to its demo blog', async () => {
    await paint();

    const link = screen.getByLabelText('See quill on a demo blog');
    expect(link).toHaveAttribute('href', 'https://templates-quill.plym.space/');
    expect(link).toHaveAttribute('target', '_blank');
    expect(link).toHaveAttribute('rel', 'noopener noreferrer');
    // The live one is a template like any other; it has a demo too.
    expect(screen.getByLabelText('See atlas on a demo blog')).toBeInTheDocument();
  });

  it('links nothing plym does not publish a demo for', async () => {
    // A private template is the tenant's own, and `retired` is on the screen
    // only because the blog is running it — no registry offers either.
    getTemplates.mockResolvedValue(
      catalog({ available: ['retired'], active: 'retired', public: [], private: ['acme'] }),
    );
    await paint('retired', ['retired']);

    expect(screen.queryByLabelText('See retired on a demo blog')).not.toBeInTheDocument();
    expect(screen.queryByLabelText('See acme on a demo blog')).not.toBeInTheDocument();
  });
