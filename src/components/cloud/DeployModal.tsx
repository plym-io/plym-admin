...
export function DeployModal({ open, patch, onClose, onApplied }: Props) {
  ...
  /**
   * Where this panel and its gateway end up, when the batch moves the blog.
   * The gateway's own resolved value wins over the draft — it is the one that
   * has been through whatever normalising the plan does to a path — and a
   * prefix that resolves to the one we are already on is not a move at all.
   */
  const move = useMemo(() => {
    if (!(PREFIX_KEY in patch)) return null;
    const next =
      plan?.changes.find((c) => c.key === PREFIX_KEY)?.to ?? patch[PREFIX_KEY];
    return typeof next === 'string' ? panelMove(next) : null;
  }, [plan, patch]);
  ...
  const settle = (outcome: OpOutcome) => {
    ...
    if (move) setMoved(outcome);
    else if (outcome === 'lost') toast.error('Lost contact with the deploy.');
  ...
  const close = () => {
    ...
    if (moved && move) {
      window.location.replace(move.adminUrl);
      return;
    }
    if (opId && !finished) {
      toast(
        move
          ? 'Deploy is still running — this page’s address changes when it lands.'
          : 'Deploy is still running — it will finish on its own.',
      );
    }
    onClose();
    // Behind this dialog is a screen that reads the gateway at the address this
    // deploy is taking away. Reloading it would only ask the mount that is
    // going, and paint its 404 over a deploy that is doing exactly what it said.
    if (opId && !move) onApplied();
  };
      ...
      <div className="p-5">
        ...
        {!opId && (plan || planError) && (
          <>
            ...
            {move && <PanelMoveNotice move={move} className="mt-4" />}
          </>
        )}
        ...
        {moved && move && <PanelMoved move={move} outcome={moved} className="mt-4" />}
        ...
        <div className="mt-5 flex items-center justify-end gap-2">
          {!opId ? (
            ...
            // Once the panel has moved, the card above carries the only button
            // worth pressing. A second one here would be the same navigation
